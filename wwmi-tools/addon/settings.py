...
class WWMI_Settings(bpy.types.PropertyGroup):
    ...
    tool_mode: bpy.props.EnumProperty(
        ...
        items=[
            ('EXPORT_MOD', '导出模组', 'Export selected collection as WWMI mod'),
            ('IMPORT_OBJECT', '导入对象', 'Import .ib ad .vb files from selected directory'),
            ('EXTRACT_FRAME_DATA', '从帧转储中提取对象', 'Extract components of all WWMI-compatible objects from the selected frame dump directory'),
            ('TOOLS_MODE', '工具箱', 'Bunch of useful object actions'),
        ],
    ...
    color_storage: bpy.props.EnumProperty(
        ...
        items=[
            ('LINEAR', '线性', 'Display vertex colors as they actually are and store them with full float precision. Handle colors via `color_attributes`'),
            ('LEGACY', 'sRGB (旧版模式)', 'Display vertex colors as sRGB shifted and store them with 8-bit float precision. Handle colors via deprecated `vertex_colors`'),
        ],
        default='LINEAR',
    ) # type: ignore
    ...
    mod_skeleton_type: bpy.props.EnumProperty(
        ...
        items=[
            ('MERGED', '合并', 'Mesh with this skeleton should have unified list of Vertex Groups'),
            ('COMPONENT', '按组件', 'Mesh with this skeleton should have its Vertex Groups split into per-component lists.'),
        ],
        default=0,
    ) # type: ignore
